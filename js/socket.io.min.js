...
// io가 기존 소스에 없을경우 {} 오브젝트가 된다.
// io 가 기존 소스에 있을경우(undefined)가 아닐경우 module.exports가 된다는듯?
var io = "undefined" == typeof module ? {} : module.exports;
(function () {
    (function (a, b) {
       
		 
	      // 여기서 a즉 c는 나중에 io가 넣어질것이다.	
				// 보통 var socket = io.connect('주소') 이런식으로 시작한다
		  	var c = a;
				
				// 현재 소스코드의 버전정보를 나타낸다 
        c.version = "0.9.16", 

			  // 이건 왜 있는지...아직은 모르겠다. 접속 on -off 인가?
			  c.protocol = 1, 
			  
			  // 4개의 객체로 구성되어있다. 통신방식이다.
			  // ["websocket", "htmlfile", "xhr-polling", jsonp-polling"]
		   	c.transports = [],
			 
			  // 뭔지 모르겠다. length : 0이라는 객체가 들어있네?
		   	c.j = [], 

			  // io.connect(주소) 하는데...여기서 주소값이 객체로 저장이된다
				// 그리고 이 주소값은 다음의 메소드및 파라미터들을 가진다
				// $events : Object  //아무것도없음
				// buffer : Array[0] //length : 0
				// closeTimeout : 6000
				// connectTimeoutTimer :2
				// connected : true
				// connecting :false
				// doBuffer : false
				// heartbeatTimeout : 6000
				// heartbeatTimeoutTimer :12
				// namspace : Object SocketNamespace
				// open : true
				// options : Object //뭔가 굉장히 많음...
				// origTransports : Array[4] //전송방식들 
				// reconnecting :false
				// sessionid : 세션아이디 브라우저가 동기화될때마다 당연히 바뀐다 
				// transport : WS //closeTimeout:13, isopen:true, sessid : 세션아이디
				// transports : Array[4]
			  c.sockets = {}, 
		    ...
		    // var socket = io.connect(serverUrl);
				// 이런식으로 사용한다. 즉, 서버의 url을 객체로 만들고 여기에 다양한 메소드와 프로퍼티를 설정하는 방식이다.
			  c.connect = function (a, d) {
					  
					  // parseUri는 자바스크립트 파서이다.
